...
def convert_questions(input_file, output_file):
    ...
    questions = []
    seen_questions = set()  # Множество для хранения уникальных вопросов
    question_id = 1
    duplicates = 0
    skipped = 0
    total_blocks = 0
    ...
    try:
        ...
        for index, block in enumerate(blocks):
            if '{' not in block:
                skipped += 1
                print(f"Пропущен (нет '{{' в блоке) на блоке {index + 1}: {block[:100]}...")
                continue
            ...
            if not question or not options_text:
                skipped += 1
                print(f"Пропущен (пустой вопрос или варианты) на блоке {index + 1}: {block[:100]}...")
                continue
            ...
            seen_questions.add(question_lower)  # Добавляем вопрос в множество
            ...
            if options and correct_answer:
                ...
                skipped += 1
                print(f"Пропущен (нет вариантов/ответа) на блоке {index + 1}: {block[:100]}...")
                
                # Выводим предыдущий и следующий вопрос, если они существуют
                if index > 0:
                    print(f"Предыдущий вопрос: {blocks[index - 1][:100]}...")
                if index < total_blocks - 1:
                    print(f"Следующий вопрос: {blocks[index + 1][:100]}...")

        # Сохраняем результат
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
        js_content = f"const modularSubjectsQuestionsDB = {json.dumps(questions, ensure_ascii=False, indent=4)};"
...
if __name__ == "__main__":
    convert_questions("modular_subjects_questions.txt", "ModularSubjects.js")